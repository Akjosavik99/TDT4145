...
CREATE TABLE
    StasjonITogrute (
        ...
        CONSTRAINT 'ende/start' CHECK(
            stasjonsType IN ('start', 'ende')
        ),
        FOREIGN KEY (ruteID) REFERENCES Togrute(ruteID) ON DELETE CASCADE,
        FOREIGN KEY (stasjonID) REFERENCES Stasjon(stasjonID) ON DELETE CASCADE,
        PRIMARY KEY (ruteID, stasjonID)
...
CREATE TABLE
    Vogn(
        ...
        CONSTRAINT 'sitte/sove' CHECK(vognType IN ('sitte', 'sove')) PRIMARY KEY(vognID)
    );
