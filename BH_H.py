...
def BH_h():
    ...
    c.execute("""
    SELECT 
    tf.ruteID, tf.dato, bs.stasjonsType, s.stasjonsnavn, bs.delstrekningID, t.hovedretning
    FROM Kunde AS k
    ...
    LEFT JOIN Togruteforekomst AS tf ON tf.forekomstID = b.forekomstID
    LEFT JOIN BestarAvStasjon AS bs ON bs.delstrekningID = sb.delstrekningID
    LEFT JOIN Stasjon AS s ON s.stasjonID = bs.stasjonID
    LEFT JOIN Togrute AS t ON t.ruteID = tf.ruteID 
    WHERE k.tlf = :tlf
    ORDER BY tf.dato, bs.delstrekningID ASC
    """, {"tlf": tlf})
    ...
    for i in range(len(res)):
        # [ruteID, dato, stasjonsType, stasjonsnavn, hovedretning]
        sammeTur = []
        ...
        for j in range(i, len(res)):
            if (res[i][1] == res[j][1]):
                ...
                break
        # Setter start og sluttstasjon
        if (sammeTur[0][2] == "start" and sammeTur[0][-1] == 1):
            startStasjon = sammeTur[0][3]
            sluttStasjon = sammeTur[-1][3]
        else:
            startStasjon = sammeTur[-1][3]
            sluttStasjon = sammeTur[0][3]
